...
function TableRowComponent({ currency, onToggleSelect }) {

  const isSelected = currency.isSelected;

  return (
    <Table.Row height={40}>
      <Table.TextCell flexBasis={100} flexShrink={0} flexGrow={0}>
        <svg className={`inline-block cursor-pointer hover:fill-yellow-300 ${isSelected ? 'fill-yellow-300 11111111111111' : ''}`} onClick={onToggleSelect} width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
